...
// 数组转树结构
export const toTree = (data: TreeItem[], str1: string = 'id', str2: string = 'pid'): TreeItem[] => {
...
export const keyEqualStr = (
    ...
    arr.forEach((item) => {
        ...
        if (item.children && (item.children as FlattenItem[]).length) {
            result = keyEqualStr(item.children as FlattenItem[], key, str, result);
        }
    });
    return result;
};
...
export const findParten = (
    ...
    arr.map((v) => {
        const res = v.children && (v.children as FlattenItem[]).some((i) => i[key] === str);
        if (res) _obj = v;
    });
    return _obj;
};
...
export const phone_patter = /^1[3-9]\d{9}$/;

// 信用代码强校验
export const xydm_patter = /^([0-9A-HJ-NPQRTUWXY]{2}\d{6}[0-9A-HJ-NPQRTUWXY]{10}|[1-9]\d{14})$/;
...
const checkCode = (val: string): boolean => {
    ...
    if (p.test(val)) {
        let sum = 0;
        for (let i = 0; i < 17; i++) {
            sum += Number.parseInt(val[i] ?? '0', 10) * (factor[i] ?? 0);
        }
...
export const idcard_patter = (val: string): boolean => {
    return checkCode(val) && checkDate(val.substring(6, 14)) && checkProv(val.substring(0, 2));
};
...
export const getColorType = (hex: string = '#666666'): boolean => {
    ...
    const level =
        Number.parseFloat(rgbList[0] ?? '0') * 0.299 +
        Number.parseFloat(rgbList[1] ?? '0') * 0.587 +
        Number.parseFloat(rgbList[2] ?? '0') * 0.114;
    return level >= 192;
};
...
export const copyText = async (text: string): Promise<void> => {
    ...
    if (navigator.clipboard?.writeText) {
        try {
            ...
            /* clipboard API 不可用（如非 https 环境），降级处理 */
        }
    }
    if (!copied) {
        // 降级方案：隐藏 textarea + execCommand（旧浏览器）
